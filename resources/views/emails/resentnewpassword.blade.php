    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 0; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); overflow: hidden;">
        ...
        <div style="padding: 20px;">
           <h2 style="color: #333;font-size: 24px;font-weight: 600;">Hi {{$name}},</h2>
            <p style="color: #555;font-size: 14px;font-weight: bold;">Your verification code is:</p>
            <div class="verification-code" style="margin: 20px 0;">
               <?php 
                    $val = $otp; 
                    $val_str = (string)$val;
                    for ($i = 0; $i < strlen($val_str); $i++)
                    {
                                   echo "<span style='font-size: 24px;font-weight: 600;color: #10241B;padding: 10px 20px;margin: 0 10px 0px 0px;background: #892CDC0D;border-radius: 5px;'>" . $val_str[$i] . "</span>";
                                }
               ?>
            </div>
            <p style="color: #555;font-size: 14px;">Enter this code in the login screen to continue. This code is valid for 10 minutes.</p>
